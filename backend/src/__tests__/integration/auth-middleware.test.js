...
/**
 * Safety check function to ensure Redis operations are safe for testing
 * @param {Object} redisClient - The Redis client instance
 * @throws {Error} If the operation is unsafe
 */
function ensureTestEnvironmentSafety(redisClient) {
	// Check environment
	if (process.env.NODE_ENV !== 'test') {
		throw new Error(
			`Unsafe operation: Cannot run tests in environment '${process.env.NODE_ENV}'. Tests must run in 'test' environment to prevent accidental data loss.`
		);
	}
	...
	if (!redisClient.isOpen) {
		throw new Error(
			'Redis client is not connected. Cannot safely flush database.'
		);
	}

	// Check Redis URL for test indicators
	const redisUrl = process.env.REDIS_URL;
	if (
		redisUrl &&
		!redisUrl.includes('test') &&
		!redisUrl.includes('/1') &&
		!redisUrl.includes('/2')
	) {
		console.warn(
			'⚠️  Warning: Redis URL does not contain test indicators. Ensure you are connected to a test database.'
		);
		// In a real production system, you might want to throw an error here
		// For now, we'll just warn and continue since the NODE_ENV check should be sufficient
	}
}
...
describe('Authentication Middleware Integration Tests', () => {
	...
	beforeEach(async () => {
		// Safety check: Ensure we're in test environment and connected to test database
		ensureTestEnvironmentSafety(redisClient);

		// Clear Redis test data before each test
		console.log('Clearing Redis test data...');
		await redisClient.flushDb();
		console.log('✅ Redis test data cleared successfully');
	...
	describe('Shopify Authentication Middleware', () => {
		it('should reject requests without shop parameter', async () => {
			const response = await request(app).get('/test-shopify-auth').expect(400);
		...
		it('should reject real shop in production mode', async () => {
			...
			expect(response.body.redirectUrl).toBe(
				'/api/auth/install?shop=real-shop.myshopify.com'
			);
		});
	...
	describe('JWT Authentication Middleware', () => {
		it('should reject requests without token', async () => {
			const response = await request(app).get('/test-jwt-auth').expect(401);
