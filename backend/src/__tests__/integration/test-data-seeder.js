...
class TestDataSeeder {
	constructor() {
		...
		if (!process.env.REDIS_URL) {
			const error = new Error(
				'REDIS_URL environment variable is not defined. Please ensure .env.test is properly configured.'
			);
	...
	/**
	 * Private method to verify test environment before destructive operations
	 * @param {string} operation - Description of the operation being performed
	 * @throws {Error} If NODE_ENV is not 'test'
	 */
	_verifyTestEnvironment(operation) {
		if (process.env.NODE_ENV !== 'test') {
			const error = new Error(
				`Unsafe operation: Cannot ${operation} in environment '${process.env.NODE_ENV}'. This operation is only allowed in 'test' environment to prevent accidental data loss.`
			);
	 ...
	 */
	async initialize() {
		this._verifyTestEnvironment('initialize test data seeder');
		...
		console.log(
			`Clearing Redis data in ${process.env.NODE_ENV} environment...`
		);
	 ...
	 */
	async cleanup() {
		this._verifyTestEnvironment('cleanup test data');
		...
		try {
			...
			try {
				...
				console.error(
					'❌ Error forcefully disconnecting Redis:',
		...
		if (errors.length > 0) {
			const errorMessage = `Test environment cleanup completed with ${
				errors.length
			} error(s):\n${errors.join('\n')}`;
	 ...
	 */
	async seedAll() {
		this._verifyTestEnvironment('seed test data');
